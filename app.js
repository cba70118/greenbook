    ...
    if (otw && t) {
        ...
        if (openBets.length) {
            ...
            html += '<div style="margin-bottom:0.5rem">' + openBets.map(function(b){
                ...
                var is3Ball = market.indexOf('3-ball') >= 0;
                var isDFS = market.indexOf('dfs') >= 0;
                var isPool = market.indexOf('pool') >= 0;
                var isProp = market.indexOf('prop') >= 0 && !isT20;
                var isOutright = market.indexOf('outright') >= 0;
                var borderColor = isFRL ? '#6ba3d6' : (isT10 || isEoR1) ? '#d29922' : isT20 ? '#bc8cff' : is3Ball ? '#ff7b72' : isDFS ? '#58a6ff' : isPool ? '#58a6ff' : isProp ? '#e3b341' : 'var(--green-500)';
                var textColor = isFRL ? '#6ba3d6' : (isT10 || isEoR1) ? '#d29922' : isT20 ? '#bc8cff' : is3Ball ? '#ff7b72' : isDFS ? '#58a6ff' : isPool ? '#58a6ff' : isProp ? '#e3b341' : 'var(--green-300)';
                var suffixText = isFRL ? 'FRL' : isEoR1 ? 'R1 T10' : isT10 ? 'T10' : isT20 ? 'T20' : is3Ball ? '3-Ball R1' : isDFS ? 'DFS' : isPool ? 'Pool' : isProp ? terms : isOutright ? 'E/W' : 'BET';
    ...
    if (t.notes && t.notes.length) {
        ...
        if (boardEl) {
            ...
            openBets.forEach(function(b) {
                ...
                var label = isFRL ? 'FRL' : isEoR1 ? 'R1 T10' : isT10 ? 'T10' : isT20 ? 'T20' : is3Ball ? '3-Ball R1' : isDFS ? 'DFS' : isPool ? 'Pool' : isProp ? propLabel : isOutright ? 'E/W' : 'BET';
                var badgeColor = isFRL ? '#6ba3d6' : isEoR1 ? '#d29922' : isT10 ? '#d29922' : isT20 ? '#bc8cff' : is3Ball ? '#ff7b72' : isDFS ? '#58a6ff' : isPool ? '#58a6ff' : isProp ? '#e3b341' : 'var(--green-400)';
        ...
        var tsOrder = {'Apr 9':25,'Apr 8':24,'Apr 6':23,'Apr 5':22,'Apr 4 R3':21,'Apr 4':20,'Apr 3 PM':19,'Apr 3':18,'Apr 2 R1':17,'Apr 2 AM':16,'Apr 1 Eve':15,'Apr 1 PM':14,'Apr 1':13,'Mar 31 Eve':12,'Mar 31 PM':11,'Mar 31 AM':10,'Mar 31':9,'Mar 30 Eve':8,'Mar 30 PM':7,'Mar 30 AM':6,'Mar 30':5,'Mar 29':4,'Mar 28':3,'':0};
...
(function() {
    ...
    openBets.forEach(function(b) {
        ...
        else if (m.indexOf('3-Ball') >= 0 || m.indexOf('Parlay') >= 0) m = '3-Ball';
        else if (m.indexOf('Prop') >= 0) m = 'Props';
        else if (m.indexOf('EoR1') >= 0) m = 'Props';
        else if (m.indexOf('DFS') >= 0) m = 'DFS';
        else if (m.indexOf('Pool') >= 0) m = 'Pool';
        else m = 'Outright';
